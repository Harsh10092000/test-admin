...
$userChoices = [
    'fruit_cart' => array_slice($orderIDs, 0, 7), // 10 users chose fruit cart
    'book_cart' => array_slice($orderIDs, 7, 11), // 8 users chose book cart
    'soup_cart' => array_slice($orderIDs, 18, 7), // 7 users chose soup cart
    'chaat_cart' => array_slice($orderIDs, 25, 9), // 9 users chose chaat cart
    'vegetable_cart' => array_slice($orderIDs, 34, 12), // 12 users chose vegetable cart
    'golgappe_cart' => array_slice($orderIDs, 46, 25), // 25 users chose golgappe cart
];
...
function allocateCarts($users, $limit) {
    ...
    $randomKeys = array_rand($users, $limit);
    return array_map(function($key) use ($users) {
        return $users[$key];
    }, $randomKeys);
}
